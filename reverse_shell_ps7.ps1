...
$output_redirector = {
    ...
    # .NET lets you stack mutex locking and releasing apparently so this is a safe little wrapper using a flag to prevent double locking and freeing
    $locked = $false
...
while ($true)
{
    ...
    if ($jobs_failed)
    {
        $jobs_failed | ForEach-Object { Receive-Job -Job $_ -ErrorAction SilentlyContinue }
        break
    }
